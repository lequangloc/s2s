...
%token EOF AS ASSERT CHECKSAT DECLAREFUN DECLAREVARIABLE DECLARESORT DEFINEFUN DEFINESORT EXCLIMATIONPT EXISTS EXIT FORALL GETASSERT GETASSIGN GETINFO GETMODEL GETOPTION GETPROOF GETUNSATCORE GETVALUE LET LPAREN POP PUSH RPAREN SETINFO SETLOGIC SETOPTION UNDERSCORE
...
command:
| cur_position LPAREN DECLAREVARIABLE symbol sort RPAREN { CommandDeclareVariable($1, $4, $5) }
...
command:
| cur_position LPAREN GETMODEL RPAREN { CommandGetModel($1) }
