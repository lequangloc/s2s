...
let vars = ref ([]: (Smtlib_util.pd * symbol * sort) list)
let asserts = ref ([]: (Smtlib_util.pd * term) list)
...
and pp_command = function 
   ...
   | CommandDefineSort (_ , symbol3 , commanddefinesort_command_symbol115 , sort7) ->
         print_string "(";print_string " ";
         print_string "define-sort";
         print_string " ";
         pp_symbol symbol3;print_string " ";
         print_string "(";print_string " ";
         pp_commanddefinesort_command_symbol11 commanddefinesort_command_symbol115;print_string " ";
         print_string ")";
         print_string " "; pp_sort sort7;print_string " "; print_string ")"; () 
   | CommandDeclareFun (_ , symbol3 , commanddeclarefun_command_sort135 , sort7) ->
        print_string "(";
         print_string " ";
         print_string "declare-fun";
         print_string " ";
         pp_symbol symbol3;print_string " "; print_string "(";print_string " "; pp_commanddeclarefun_command_sort13 commanddeclarefun_command_sort135;print_string " "; print_string ")";print_string " "; pp_sort sort7;print_string " "; print_string ")"; () 
   | CommandDeclareVariable (p , symbol3 , sort7) -> begin
         let () = if !Globals.to_smt2 then
           let () = print_string "(" in
           let () = print_string " " in
           let ()= print_string "declare-fun" in (* "declare-variable" *)
           let () = print_string " " in
           print_string " "
          else () in
         let () = pp_symbol symbol3 in
         let () = if !Globals.to_smt2 then
         print_string " () "
         else () in
         pp_sort sort7;
         let () = vars := !vars@[(p,symbol3,sort7)] in
         let () = if !Globals.to_smt2 then
           let () = print_string " )\n" in () (* ; print_endline ")" *)
         else () in
()
   ...
   | CommandDefineFun (_ , symbol3 , commanddefinefun_command_sortedvar155 , sort7 , term8) ->
         print_string "(";print_string " ";
         print_string "define-fun";print_string " ";
         pp_symbol symbol3;print_string " ";
         print_string "(";print_string " ";
         pp_commanddefinefun_command_sortedvar15 commanddefinefun_command_sortedvar155;
         print_string " "; print_string ")";print_string " ";
         pp_sort sort7;print_string " ";
         pp_term term8;print_string " "; print_string ")"; () 
   ...
   | CommandAssert (p , term3) -> let () = if !Globals.to_smt2 then
       let () = print_string "( " in
       let () = print_string "assert " in ()
     else () in
     let () = asserts := !asserts@[(p,term3)] in
     let () = pp_term term3 in
     let () = if !Globals.to_smt2 then
       let () = print_string " )\n" in
       ()
     else () in
     ()
   | CommandCheckSat (_) ->  if !Globals.to_smt2 then
       let () = print_string "(" in
       let () = print_string " " in
       let () = print_string "check-sat" in
       let () = print_string " " in
       print_string ")\n"
     else () 
...
and pp_commands = function 
   | Commands (_ , commands_commands_command301) ->
         pp_commands_commands_command30 commands_commands_command301; ()
...
and pp_specconstant = function 
   ...
   |SpecConstNum (_ , str1) -> let () = if !Globals.to_smt2 then
          print_string str1
        else () in () 
   |SpecConstString (_ , str1) -> let () = if !Globals.to_smt2 then
          print_string str1
        else () in () 
   |SpecConstsHex (_ , str1) -> let () = if !Globals.to_smt2 then
          print_string str1
        else () in () 
   |SpecConstsBinary (_ , str1) -> let () = if !Globals.to_smt2 then
          print_string str1
        else () in ()
...
and pp_symbol = function 
   |Symbol (_ , str1) -> let () =
       if !Globals.to_smt2 then
         let str2 =
           if String.compare str1 "Concat" ==0 then
             if !Globals.to_smt2_trau then
               str1
             else
               "str.++"
           else if String.compare str1 "Length" ==0 then
             if !Globals.to_smt2_trau then
               str1
             else
               "str.len"
...
and pp_term = function 
   | TermSpecConst (_ , specconstant1) ->
         ...
         in pp_specconstant specconstant1; () 
   ...
   | TermQualIdTerm (_ , qualidentifier2 , termqualidterm_term_term563) ->
         ...
         let () = pp_qualidentifier qualidentifier2 in
         (* print_string ""; *)
         let () = pp_termqualidterm_term_term56 termqualidterm_term_term563 in
         let () = if !Globals.to_smt2 then
           print_string " )"
         else () in
         () 
