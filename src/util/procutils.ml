...
struct
  ...
  let maybe_raise_timeout (fn: 'a -> 'b) (arg: 'a) (limit:float) : 'b =
    ...
    try
      let () = if !VarGen.enable_time_stats then Timelog.logtime # timer_start proof_no limit else () in
      let res = fn arg in
      let x = Unix.getitimer Unix.ITIMER_REAL in
      (* let nt = limit -. x.Unix.it_value in *)
      let nt = limit -. x.Unix.it_value in
      let () = if !VarGen.enable_time_stats then Timelog.logtime # timer_stop proof_no nt else () in
      set_timer 0.0;
      reset_sigalrm ();
      res
    with e ->
...
let append_log_out flag fn s =
  if not flag then () else begin
    (try
      Unix.mkdir "logs" 0o750
    with _ -> ());
    let oc = open_out_gen [Open_creat; Open_text; Open_append] 0o640 ("logs/"^fn) in
    output_string oc s;
    flush oc;
    close_out oc
  end
  ...
  let start (log_all_flag: bool) (log_file: string) (prover: string * string * string array) set_process prelude= 
    let (prover_name, prover_proc, prover_arg_array) = prover in
    let () = append_log_out log_all_flag log_file ("["^prover_name^".ml]: >> Starting "^prover_name^"...\n") in
    try
    ...
    | e -> begin
        ...
        append_log_out log_all_flag log_file ("["^prover_name^".ml]: >> Error while starting "^prover_name ^ "\n");
        raise e
      end
  ...
  let stop (log_all_flag: bool) (log_file: string) (process:proc) (invocations: int) (killing_signal: int) ending_function =
    let () = ending_function () in
    let () = append_log_out log_all_flag log_file ("\n[" ^ process.name  ^ ".ml]: >> Stop " ^ process.name ^ " after ... " ^ (string_of_int invocations) ^ " invocations\n") in
    (* flush log_file; *)
    (* close_pipes process; *)
    let fn () =
  ...
  let restart (log_all_flag: bool) (log_file: string) (reason:string) (prover_name: string) start stop =
    append_log_out log_all_flag log_file ("[" ^ prover_name ^ ".ml]: >> Restarting " ^ prover_name ^ " because of: " ^ reason) ;
    stop ();
    start ()
