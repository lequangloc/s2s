...
let proof_search_left_unfold_pred_heap_contra cpreds e lvn leqs ldiseqs rdiseqs l_allocas l_nulls r_nulls =
  let r, sst, s = proof_search_left_unfold_pred cpreds e lvn leqs ldiseqs rdiseqs l_allocas l_nulls r_nulls in
  match r with
    | [ls] ->
          if s == ENorm then
            (* check whether emp |- pto
               force LU for other cases
            *)    let is_contra_bases, rem = List.partition (fun (e,_) ->
                SLND.is_empty_heap_or_htrue e.ENT.ante &&
                    not (SLND.is_empty_heap_or_htrue e.ENT.cons)
            ) ls in
            let ls1 = if is_contra_bases == [] then
              ls
            else
              is_contra_bases@(List.map (fun (e, pr) -> ({e with ENT.b_lu = true;}, pr)) rem)
            in
            [ls1], sst, s
          else r, sst, s
    | _ -> r, sst, s

let search_pred_with_match_alloca_heap_contra cpreds e lvns qf_rptos leqs lneqs rneqs l_allocas l_nulls r_nulls=
  let lunfold_lbls, _ = search_pred_with_match_alloca cpreds false lvns leqs ((List.map (fun n -> n.PoN.hpto_node) qf_rptos)@r_nulls) in
  let ls= proof_search_unfold cpreds e true lneqs rneqs l_allocas l_nulls r_nulls lunfold_lbls in
  let is_contra_bases, rem = List.partition (fun (e,_) ->
      SLND.is_empty_heap_or_htrue e.ENT.ante &&
          not (SLND.is_empty_heap_or_htrue e.ENT.cons)
  ) ls in
...
let ps_composition_linear_pred cpreds e lvn lptos leqs ldiseqs rdiseqs l_nulls rvn reqs r_nulls
  ...
  let ls_left_composes = proof_search_composition_left_unfold cpreds e (lvn, rvn) in
  let rs = List.map (fun r -> [r]) (ls_left_composes@ls_composes) in
  (* reset inst flag of inst_rvns *)
  let n_cons = SLND.reset_rinst e.ENT.cons [rvn] in
  let ne = {e with ENT.cons = n_cons} in
  ([[(ne, IR_RNON_INST [rvn])]])@rs, [] , ENorm



let ps_pred_pred cpreds e cyc_matched_preds lvns lptos leqs ldiseqs rdiseqs l_nulls rvns reqs  r_nulls rqvars
      l_allocas rq_allocas fpvns fp_allocas=
  ...
  match m_same_preds with
    | (lvn,rvn)::_ -> begin
        ...
        if not rvn.PeN.hpred_rhs_inst then
          let r= proof_search_left_unfold_pred_heap_contra cpreds e lvn leqs ldiseqs rdiseqs l_allocas l_nulls r_nulls in
          true, r
        else
        ...
        if is_ok then
          ...
          if neq_precise_sst != []  then
            ...
            let () = Debug.ninfo_hprint (add_str "ex_cond_sst" (pr_list Var.string_of_pair)) ex_cond_sst no_pos in
            (*to check condition: all RHS vars are quantified *)
            let qf_rallocas = Var.intersect rqvars rq_allocas in
            if List.for_all (fun sv -> not (Var.mem sv qf_rallocas)) rvn.PeN.hpred_arguments then
              if ex_cond_sst ==[] then
...
let heur_arith_lunfold cpreds e lptos lvns rptos rvns leqs lneqs rneqs l_allocas l_nulls r_nulls=
  match lvns with
    | [lvn] ->
          if e.ENT.b_lu then
            true, proof_search_left_unfold_pred_heap_contra cpreds ({e with ENT.b_lu = false;}) lvn leqs lneqs rneqs l_allocas l_nulls r_nulls
          else begin
            if (* lvn.PeN.hpred_unfold_num == 0 && *)
              (List.length rvns == 1) && (List.length lptos == List.length rptos) then
                true, proof_search_left_unfold_pred_heap_contra cpreds e lvn leqs lneqs rneqs l_allocas l_nulls r_nulls
            else (false, ([],[], ENorm))
          end
    | _ -> false, ([],[], ENorm)

let ps_ex_x cpreds is_ho e =
  ...
  match m_ptos with
    ...
    | [] -> begin (* BEGIN: not any matched ptos *)
        (*match preds *)
        let is_ok, r = heur_arith_lunfold cpreds e lptos lvns rptos rvns leqs lneqs rneqs l_allocas l_nulls r_nulls in
        if is_ok then r else
        ...
        match (m_acpreds@m_cpreds_end) with
            ...
            | [] -> begin (* BEGIN: not any matched pred *)
                ...
                let lunfold_lbls = Cpred.search_pred_base_linear_unfold cpreds lvns leqs (l_nulls) in
                if lunfold_lbls !=[] then
                ...
                if runfold_lbls != [] then
                  ...
                  if rbrs == [] then [], [], ERuEmp
                    ...
                    let r = List.map (fun r -> [r]) rbrs in
                    r,[],ENorm
                else begin
                  try
                    (* EX-MID + RInd *)
                  ...
                  with Not_found -> begin
                    ...
                    let is_applied, r = ps_pred_pred cpreds e m_cpreds lvns lptos leqs lneqs rneqs l_nulls rvns reqs r_nulls (rqvars0)
                        l_allocas q_rallocals fpvns fp_allocas in
                    if is_applied then
                      r
                    else
                      try
                        (* find a Left pred-pto for unfolding *)
                        search_pred_with_match_alloca_heap_contra cpreds e lvns qf_rptos leqs lneqs rneqs l_allocas l_nulls r_nulls
                      with Not_found ->
                      (* empty heap in LHS? *)
                      if lvns==[] && lptos==[] then
...
let ps_ex cpreds is_ho e=
  ...
  Debug.no_1 "ps_ex" pr1 pr_out
      (fun _ -> ps_ex_x cpreds is_ho e) e
