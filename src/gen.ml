...
struct
  ...
  let silenced_print f s = if !VarGen.quiet_mode (* !silence_output *) then () else f s 
  ...
  let print_endline_quiet s = VarGen.print_endline_quiet s
  (*   let flag = !VarGen.quiet_mode (\* !silence_output *\)(\* compete_mode *\) in *)
  ...
  let print_string_quiet s =
    if !VarGen.quiet_mode (* !silence_output *) (* compete_mode *) then ()
    else print_string s
  ...
  let print_web_mode s = 
    if !web_mode then print_endline s
    else print_endline_quiet s 

  let print_string_web_mode s = 
    if !web_mode then print_string s
...
struct
  ...
  let rec remove_dups_eq eq n =
    match n with
      [] -> []
    | q::qs -> if (mem_eq eq q qs) then
        remove_dups_eq eq qs
  ...
  let remove_fst_eq eq x l2 =
    let rec aux ls res=
      match ls with
        | x1::rest -> if eq x x1 then (res@rest)
          else aux rest (res@[x1])
        | [] -> res
    in
    aux l2 []
