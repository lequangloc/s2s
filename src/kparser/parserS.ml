 ...
 SHGram.Entry.of_parser "peek_try" 
    (fun strm -> 
	 ...
	 | [GT,_; CBRACE,_] -> raise Stream.Failure (*vp*)
         ...
         | [GT,_;BND,_] -> raise Stream.Failure 
...
cexp_w:
  ...
  | "dang_var"
          [ `IDENTIFIER id ; `DANG ->  Pure_f (P.BForm ((P.mkDang id Unprimed (get_pos_camlp4 _loc 1), None), None)) ]
  | "una"
    ...
    | t = cid ->
        ...
        if String.contains id '.' then
          ...
          if mu=Globals.level_name then
            ...
            (* let () = print_endline (String.concat "." strs) in *)
            ( match strs with
              | [id;fid] ->
                    Pure_c (P.VarAcc ((id,p), fid, false, get_pos_camlp4 _loc 1))
              | _ ->  Pure_c (P.Var (t, get_pos_camlp4 _loc 1))
            )
        else
    ...
    | `BND -> Pure_c (P.AConst(Bnd, get_pos_camlp4 _loc 1))
 ...
 [[ `OPAREN; e=expression; `CPAREN; ue=unary_expression_not_plusminus ->
  ...
  | `OPAREN; `FLOAT; `CPAREN; t=unary_expression ->
      Cast { exp_cast_target_type = Float;
             exp_cast_body = t;
             exp_cast_pos = get_pos_camlp4 _loc 1 }
  | `OPAREN; `DBL; `CPAREN; t=unary_expression ->
      Cast { exp_cast_target_type = Float;
             exp_cast_body = t;
             exp_cast_pos = get_pos_camlp4 _loc 1 }]];
  ...
  let proc_source = 
    if String.compare id Globals.nondet_int_proc_name == 0 then 
      ...
      Some (
        ...
        "  requires true\n" ^
        "  ensures res = 0;\n")
    else  if String.compare id Globals.nondet_uint_proc_name == 0 then 
      let () = rel_names # push Globals.nondet_uint_rel_name in
      Some (
        "int " ^ Globals.nondet_uint_proc_name ^ "()\n" ^
        "  requires true\n" ^
        "  ensures true & " ^ Globals.nondet_uint_rel_name ^ "(res)" ^ ";\n")
    else None
...
let add_tnt_prim_proc prog id = 
  if String.compare id Globals.nondet_int_proc_name == 0 then
    ...
    in
    let () = rel_names # push Globals.nondet_int_rel_name in
    let proc_decl = parse_c_aux_proc nondet_int_proc_name proc_src in
    { prog with
    ...
    let proc_src =
      ...
      "  ensures res = 0;\n"
    in
    let proc_decl = parse_c_aux_proc "__VERIFIER_error" proc_src in
    { prog with Iast.prog_proc_decls = prog.Iast.prog_proc_decls @ [proc_decl]; }
  else if String.compare id "__VERIFIER_nondet_uint" == 0 then 
    let proc_src =
      "int __VERIFIER_nondet_uint()\n" ^
      "  requires true\n" ^
      "  ensures  0=0  & " ^ Globals.nondet_uint_rel_name ^ "(res)" ^ ";\n"
    in
    let nondet_rel = {
      rel_name = nondet_uint_rel_name;
      rel_typed_vars = [(int_type, nondet_uint_rel_name ^ "res")];
      rel_formula = P.mkTrue no_pos; }
    in
    let () = rel_names # push Globals.nondet_uint_rel_name in
    let proc_decl = parse_c_aux_proc Globals.nondet_uint_rel_name proc_src in
    { prog with
      Iast.prog_rel_decls = prog.Iast.prog_rel_decls @ [nondet_rel];
      Iast.prog_proc_decls = prog.Iast.prog_proc_decls @ [proc_decl]; }
  else prog
