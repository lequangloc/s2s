...
# Created by Z3-str group
# Reused by S3 group
# Used to 
#   1. convert the input to the format that S3 supports
#   2. verify the output for the purpose of testing
...
def genSolAsserts():
  ...
  for name, value in varSolution.items():
    if (name == "u"):
      ...
      continue
    elif (varTypeDict[name] == "string" and value.find('!val!') != -1):
      result.append("(assert (= " + name + " \"" + "@" + "\" ) )" + "\n")
    elif (varTypeDict[name] == "string" and value != '""'):
...
if __name__ == '__main__':
  ...
  try:
    ...
    #   > SAT, verify the answer
    #   > UNSAT/UNKNOWN, report
    # --------------------------------------------------
    err = subprocess.check_output(paras, );
    outStr, hasError = processOutput(err)
...
#    el
    if outStr.find(">> SAT") != -1: 
      ...
      paras = [solver, "-v", convertedVerifyInputFilename]
      
      # run the solver again, check the solution.
      err1 = subprocess.check_output(paras, );
      outStr1, hasError1 = processOutput(err1)
      if hasError1 == 1:      
        print "> Exit"
        sys.exit(0)
      if err1.find(">> SAT") != -1:
